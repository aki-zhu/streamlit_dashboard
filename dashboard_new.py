...
# Input a dataset
df = pd.read_csv(
    'camels_gb_v2_hydromet_daily_timeseries_27089_19701001-20220930.csv'
)
...
data_db = df.melt(id_vars="Date",value_vars=value_columns,var_name="Category",value_name="Value")

data_db["Category"] = data_db["Category"].astype("category")
...
with col1:
    # Category Filter
    category_filter = st.multiselect("Select Category", options=data_db["Category"].unique(), default=list(data_db["Category"].cat.categories))
